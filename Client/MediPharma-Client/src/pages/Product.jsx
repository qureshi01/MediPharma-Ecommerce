...
const Product = () => {
  ...
  const retrieveProduct = async () => {
    const response = await axios.get(
      "http://localhost:2399/api/product/id?productId=" + productId
    );
  ...
  const retrieveProductsByCategory = async () => {
    const response = await axios.get(
      "http://localhost:2399/api/product/category/fetch?categoryId=" +
        categoryId
    );
  ...
  const saveProductToCart = (userId) => {
    fetch("http://localhost:2399/api/cart/add", {
      method: "POST",
      headers: {
      ...
      <div class="row">
        ...
        <div class="col-sm-3 mt-2 admin">
          <div class="card form-card border-color custom-bg">
            <img
              src={
                "http://localhost:2399/api/product/image/" + product.imageName
              }
              style={{
                maxHeight: "500px",
                maxWidth: "100%",
